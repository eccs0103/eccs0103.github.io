...
export class DefaultMPAConfig extends ViteConfig {
	...
	static async construct(inputs: readonly URL[], rootEntries: readonly URL[], pathEntries: readonly URL[], output: URL, plugins: readonly VitePlugin[] = []): Promise<DefaultMPAConfig> {
		DefaultMPAConfig.#lock = false;
		const config = new DefaultMPAConfig(inputs, rootEntries, pathEntries, output, plugins);
