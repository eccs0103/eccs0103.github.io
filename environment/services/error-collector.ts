...
export class ErrorCollector extends Collector {
	...
	#onError(event: ErrorEvent): void {
		this.emit("js_error", JavaScriptError, new JavaScriptError(event.message, event.filename.insteadEmpty(undefined), event.lineno.insteadZero(undefined)));
	}

	#onReject(event: PromiseRejectionEvent): void {
		this.emit("js_error", JavaScriptError, new JavaScriptError(Error.from(event.reason).message, undefined, undefined));
	}
}
//#endregion
