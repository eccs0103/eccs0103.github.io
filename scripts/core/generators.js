...
class Random {
    ...
    boolean(factor = 0.5) {
        if (!Number.isFinite(factor))
            throw new TypeError(`The factor $must be a finite number`);
    ...
    subarray(array, count = 1) {
        if (!Number.isInteger(count))
            throw new TypeError(`The count $must be a finite integer number`);
        if (0 > count || count > array.length)
            throw new RangeError(`The count $is out of range [0 - $]`);
    ...
    case(cases) {
        if (1 > cases.size)
            throw new RangeError(`The cases must have at least 1 item`);
