...
class Vector1D extends Vector {
    ...
    static parse(string) {
        ...
        if (match === null)
            throw new SyntaxError(`Invalid syntax '${string}' for 1D vector`);
    ...
    static fromVector(vector) {
        ...
        return new Vector1D(metric1.done ? 0 : metric1.value);
    }
    /**
...
class Vector2D extends Vector1D {
    ...
    static parse(string) {
        ...
        if (match === null)
            throw new SyntaxError(`Invalid syntax '${string}' for 2D vector`);
    ...
    static fromVector(vector) {
        ...
        return new Vector2D(metric1.done ? 0 : metric1.value, metric2.done ? 0 : metric2.value);
    }
    /**
     * @param x The x-coordinate of the vector.
...
class Vector3D extends Vector2D {
    ...
    static parse(string) {
        ...
        if (match === null)
            throw new SyntaxError(`Invalid syntax '${string}' for 3D vector`);
    ...
    static fromVector(vector) {
        ...
        return new Vector3D(metric1.done ? 0 : metric1.value, metric2.done ? 0 : metric2.value, metric3.done ? 0 : metric3.value);
    }
    /**
     * @param x The x-coordinate of the vector.
     * @param y The y-coordinate of the vector.
...
class Timespan {
    ...
    static parse(string) {
        ...
        if (match === null)
            throw new SyntaxError(`Invalid time '${string}' syntax`);
    ...
    static viaDuration(duration = 0) {
        if (!Number.isFinite(duration))
            throw new TypeError(`The duration ${duration} must be a finite number`);
    ...
    static viaTime(negativity = false, hours = 0, minutes = 0, seconds = 0, milliseconds = 0) {
        if (!Number.isFinite(hours))
            throw new TypeError(`The hours ${hours} must be a finite number`);
        if (!Number.isFinite(minutes))
            throw new TypeError(`The minutes ${minutes} must be a finite number`);
        if (!Number.isFinite(seconds))
            throw new TypeError(`The seconds ${seconds} must be a finite number`);
        if (!Number.isFinite(milliseconds))
            throw new TypeError(`The milliseconds ${milliseconds} must be a finite number`);
        const timespan = new Timespan();
...
class Matrix {
    ...
    constructor(size, initializer) {
        if (!Vector.isInteger(size))
            throw new TypeError(`The size ${size} must be a finite integer vector`);
        if (0 > size.x || 0 > size.y)
            throw new RangeError(`The size ${size} is out of range [(0, 0) - (+∞, +∞))`);
        this.#size = size;
     ...
     */
    get(position) {
        if (!Vector.isInteger(position))
            throw new TypeError(`The position ${position} must be a finite integer vector`);
        const { x, y } = position;
        const size = this.#size;
        if (0 > x || x >= size.x || 0 > y || y >= size.y)
            throw new RangeError(`The position ${position} is out of range [(0, 0) - ${size})`);
