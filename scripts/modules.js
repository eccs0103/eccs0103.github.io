...
class Random {
	 ...
	 * @template Item Case type.
	 * @param {Map<Item, Number>} cases Map of cases.
	 * @returns Random case.
	 */
	static case(cases) {
		const summary = Array.from(cases).reduce((previous, [_case, precentage]) => previous + precentage, 0);
		const random = Random.number(0, summary);
		let selection = undefined;
		let start = 0;
		for (const [_case, precentage] of cases) {
			const end = start + precentage;
...
class Application {
	...
	/** @type {String} */ static #project = `Presonal Webpage`;
